...
function buka_form ($link, $id, $aksi){
    echo '<from methode="post" action="'.$link. '&show-action" class="form-horizontal" enctype="multipart/form-data">
            <input type="hidden" name="id" value="'.$id.'">
            <input type="hidden" name="aksi" value="'.$aksi.'">';
}

function buat_texbox($label, $nama, $nilai, $lebar='4', $tipe="text", $disabled=""){
...
function buat_chechbox ($label, $nama, $list){
    echo '<div class ="form-group" id="'.$nama.'">
...
<?php
    echo '<div class="form-group imagepicker">
        ...
        if ($nilai !="") echo'<img scr="../media/source/'.$nilai.'" width="150" stylw="margin-bottom:10px">';
    echo '</div>
    ...
    <div class="modal fade" id="modal-'.$nama.'" tabinex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
    <div class="modal-content">
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="close">
        <span aria-hideen="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">File Manager</h4>
    </div>
    <div class="modal-body">
        <iframe src="../plugin/filemanager/dialog.php?type=1&field_id='.$nama.'&relative_url=1" 
